...
//root -l -x "analisi_mu_after.cxx(\"pos_Be00mm50.dat\")"

void analisi_mu_after(string nomefile="ciao"){
  ...
  TH2D *hpxdE = new TH2D("hpxdE","px:dE_E",80,-2e-3,2e-3,80,0,0.40);
  ...
  double mu_tot=0;
  while (1) {
    in >>partID>>turn>>x>>px>>y>>py>>ct>>dE_E>>s>>E>>w>>z>>turn0>>l_tot;
    hxx->Fill(x);
    if(turn0==0)hpx->Fill(px);
    hyy->Fill(y);
    hpy->Fill(py);
    hct->Fill(ct);
    hdE->Fill(dE_E);
    if(turn0==0)hEE->Fill(E*(1+dE_E));
    hAL->Fill(partID);
    if(turn0==0)hpxx->Fill(px,x);
    hpyy->Fill(py,y);
    hpxdE->Fill(px,dE_E);
    hww->Fill(w);
    hzz->Fill(z);
  ...
  /*
   ...
   ht0->Draw();
   TCanvas *Cll = new TCanvas("l_tot [m]");
   hll->Draw();
